...
const MODES = [
  {
    id: 'describe', label: 'Descrever', desc: 'Descrição detalhada',
    icon: <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>,
  },
  {
    id: 'extract_text', label: 'Extrair Texto', desc: 'OCR — lê o texto da imagem',
  ...
  {
    id: 'analyze_chart', label: 'Analisar Gráfico', desc: 'Interpreta dados visuais',
    icon: <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>,
  },
  {
    id: 'identify', label: 'Identificar', desc: 'Objetos e elementos',
  ...
  {
    id: 'quality', label: 'Qualidade', desc: 'Avalia a imagem tecnicamente',
    icon: <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"/></svg>,
  },
  {
    id: 'custom', label: 'Pergunta livre', desc: 'Faça qualquer pergunta',
...
export default function ImageAnalyzer() {
  ...
  function handleFile(f) {
    ...
    if (!allowed.includes(f.type)) { setError('Apenas imagens JPG, PNG, GIF e WebP.'); return; }
    if (f.size > 20 * 1024 * 1024) { setError('Imagem muito grande. Máximo 20MB.'); return; }
  ...
  async function handleAnalyze() {
    ...
    if (mode === 'custom' && !question.trim()) { setError('Digite uma pergunta para o modo "Pergunta livre".'); return; }
  ...
  return (
    <div>
      ...
      <p className="page-subtitle">Envie uma imagem e a IA irá analisá-la, extrair texto, interpretar gráficos e muito mais.</p>
              ...
              <div>
                <div className="image-preview-wrapper">
                  ...
                  <div className="image-preview-overlay">
                    <span className="image-preview-name">{file.name} · {formatSize(file.size)}</span>
                  </div>
                </div>
                <button className="btn btn-secondary" style={{marginTop:10, fontSize:13, padding:'8px 14px', width:'auto'}} onClick={() => { setFile(null); setPreview(null); setResult(''); }}>
          ...
          <div className="card">
            <div className="card-header">
              ...
              <div>
                ...
                <div className="card-desc">O que a IA deve fazer com a imagem</div>
              </div>
            </div>
            <div className="modes-grid" style={{gridTemplateColumns:'1fr 1fr', marginBottom: mode === 'custom' ? 12 : 0}}>
              {MODES.map(m => (
      ...
      {result && (
        <>
          ...
          <div className="actions">
            ...
            <button className="btn btn-secondary" onClick={() => { setFile(null); setPreview(null); setResult(''); setQuestion(''); }}>
